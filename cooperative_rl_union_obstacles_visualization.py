...
from RLmain import DEFAULT_CONFIG_PATH, ensure_directory, make_env
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=11,
        help="Random seed for trajectories and obstacle placement.",
    )
    parser.add_argument(
        "--obstacles", type=int, default=3, help="Number of occluding obstacles."
    )
    parser.add_argument(
...
def generate_obstacles(
    ...
    ring_centroid = (
        ...
        else np.array(forbidden_region.centroid.coords[0], dtype=np.float32)
    )
    target_radius = None
    if forbidden_region is not None:
        ...
        target_radius = np.array(
            [ring_radius_x + 18.0, ring_radius_y + 18.0], dtype=np.float32
        )
    ...
    for _ in range(500):
        ...
        translated = affinity.translate(
            rotated, xoff=float(center[0] + dx), yoff=float(center[1] + dy)
        )
...
def push_out_of_obstacles(
    ...
    for robot_idx in range(len(adjusted)):
        for _ in range(12):
            ...
            adjusted[robot_idx] = clamp_positions(
                adjusted[robot_idx], env.world_low, env.world_high
            )
...
def safe_positions(
    ...
    adjusted = separate_robot_bodies(
        env, adjusted, headings, env.world_low, env.world_high
    )
    adjusted = push_out_of_obstacles(env, adjusted, obstacles, margin=CLEARANCE_MARGIN)
    return adjusted
...
def metrics_with_obstacles(
    env, robots, visible_polygons, obstacles
) -> dict[str, float]:
    obstacle_union = (
        ops.unary_union(obstacles) if obstacles else shapely.GeometryCollection()
    )
    ...
    for robot_idx, robot_sensors in enumerate(robots):
        for sensor_idx, _ in enumerate(robot_sensors):
            if env.outward_sensor_mask[robot_idx, sensor_idx]:
                polygon = visible_polygons[robot_idx][sensor_idx].intersection(
    ...
    if outward_polygons:
        union_area = ops.unary_union(outward_polygons).area / max(
            env.workspace_area, 1e-6
        )
    else:
        union_area = 0.0
    ...
    for i in range(env.robot_count):
        ...
        for j in range(env.robot_count):
            if i == j:
                continue
            target = (
                float(env.robot_positions[j, 0]),
                float(env.robot_positions[j, 1]),
            )
            if any(
                visible_pair(robots[i][sensor_idx], target, obstacle_union)
                for sensor_idx in inward_indices
            ):
                visible_pairs += 1
    ...
    reward = (
        2.4 * union_area
        + 1.8 * teammate_visibility
        - 0.35 * overlap_penalty
        - 0.10 * steering_penalty
    )
...
def main() -> None:
    ...
    def draw_frame(frame_idx: int):
        ...
        env.sensor_offsets.fill(0.0)
        fixed_robot_headings = heading_from_positions(
            env.robot_positions, previous_positions
        )
        ...
        fixed_metrics = metrics_with_obstacles(
            env, fixed_robots, fixed_visible_polygons, obstacles
        )
