    ...
    for sensor in sensors:
        for view_zone in view_zones:
            if sensor.get_polygon().intersects(view_zone.get_polygon()):
                coverage_area += (
                    sensor.get_polygon().intersection(view_zone.get_polygon()).area
                )
    ...
    for i, sensor in enumerate(sensors):
        for other_sensor in sensors[i + 1 :]:
            intersection_areas.append(
                -sensor.get_polygon().intersection(other_sensor.get_polygon()).area
                / sensor.get_polygon().union(other_sensor.get_polygon()).area
            )
...
def get_angle_density(sensors: Genotype) -> float:
    ...
    for sensor in sensors:
        middle_point = (
            (sensor.position[0] + middle_point[0]) / 2,
            (sensor.position[1] + middle_point[1]) / 2,
        )
    ...
    for i, sensor in enumerate(sensors):
        ...
        if angel < 0:
            angel += math.pi * 2
        mean_angel = (
            middle_point[0] - sensor.position[0],
            middle_point[1] - sensor.position[1],
        )
...
def fitness_function(
    sensors: Genotype, view_zones: list[ViewZone]
) -> tuple[float, float]:
