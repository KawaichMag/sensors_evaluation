...
DEFAULT_ARTIFACTS_DIR = "artifacts/cooperative_rl"
...
class CooperativeActiveSensingEnv(gym.Env):
    ...
    def __init__(
        ...
        self.target_points = np.zeros((self.target_count, 2), dtype=np.float32)
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        self.target_points = self._rng.uniform(
            low=self.world_low,
            high=self.world_high,
            size=(self.target_count, 2),
        ).astype(np.float32)
    ...
    def render(self):
        ...
        for target in self.target_points:
            ax.plot(target[0], target[1], ".", color="tab:gray", alpha=0.35)
        ...
        ax.set_title(
            "Outward coverage {:.2f} | Teammate visibility {:.2f} | Overlap {:.2f}".format(
    ...
    def _calculate_reward(self) -> RewardBreakdown:
        ...
        flat_sensors = self._flatten_sensors(robots)
    ...
    def _coverage_ratio(self, sensors: list[Sensor]) -> float:
        ...
        covered = 0
        for target in self.target_points:
            point = Point(float(target[0]), float(target[1]))
            if any(sensor.get_polygon().covers(point) for sensor in sensors):
                covered += 1

        return covered / max(1, len(self.target_points))
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        targets = self._normalize_positions(self.target_points).reshape(-1)

        observation = np.concatenate(
            [robot_positions, offsets, outward_mask, rotation_features, targets]
        ).astype(np.float32)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--targets", type=int, default=DEFAULT_TARGET_COUNT, help="Number of sampled environment targets.")
