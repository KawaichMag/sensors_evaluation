...
from RLmain import DEFAULT_CONFIG_PATH, load_configuration
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Path to saved configuration pickle.",
    )
    parser.add_argument("--output", default=DEFAULT_OUTPUT, help="Output image path.")
    parser.add_argument(
        "--sensor", type=int, default=0, help="Sensor index to highlight."
    )
    parser.add_argument(
        "--local-fov-deg",
        type=float,
        default=60.0,
        help="Debug local FOV angle in degrees.",
    )
    parser.add_argument(
        "--use-config-fov",
        action="store_true",
...
def main() -> None:
    ...
    local_fov_angle = (
        float(sensor.angle) if args.use_config_fov else math.radians(args.local_fov_deg)
    )
    ...
    sensor_origin = robot_position + rotation_matrix(robot_heading) @ np.asarray(
        sensor.position, dtype=np.float32
    )
    ...
    plot_polygon(
        local_fov, ax=ax, add_points=False, color=(*base_rgb[:3], 0.62), alpha=0.62
    )
    ax.scatter(
        robot_position[0],
        robot_position[1],
    ...
    ax.text(
        ...
        robot_position[1] + 3.0,
        "R0",
        fontsize=10,
        weight="bold",
    )
    ax.scatter(
        sensor_origin_tuple[0], sensor_origin_tuple[1], color="black", s=22, zorder=7
    )
    ax.plot(
        [
            sensor_origin_tuple[0],
            sensor_origin_tuple[0] + radius * math.cos(center_heading),
        ],
        [
            sensor_origin_tuple[1],
            sensor_origin_tuple[1] + radius * math.sin(center_heading),
        ],
