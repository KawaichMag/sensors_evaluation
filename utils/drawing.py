...
def draw_plan(drone_size: tuple[float, float], sensors: list[Sensor]) -> None:
    drone_polygon = shapely.Polygon(
        [
            (-drone_size[0] / 2, drone_size[1] / 2),
            (drone_size[0] / 2, drone_size[1] / 2),
            (drone_size[0] / 2, -drone_size[1] / 2),
            (-drone_size[0] / 2, -drone_size[1] / 2),
        ]
...
def draw_experiment(
    ...
    for sensor in population[0]:
        plot_polygon(
            sensor.get_polygon(), ax=ax, color=primary_solution_color, add_points=False
        )
        fitness = fitness_function(population[0], view_zones)
        plt.title(
            f"{first_label} = {fitness[0]:.4f}, {second_label} = {fitness[1]:.4f}"
        )
    ...
    for view_zone in view_zones:
        plot_polygon(
            view_zone.get_polygon(), ax=ax, color=view_zones_color, add_points=False
        )
...
def draw_fronts(
    ...
    pop_fitness = [
        fitness_function(population[i], view_zones) for i in range(len(population))
    ]
...
def create_gif(folder_path: str, filename: str):
    ...
    images = [
        ...
        if img.endswith(".png") and img.startswith(filename)
    ]
    images.sort(key=lambda x: int(x.split("_")[-1].split(".")[0]))
    images = [Image.open(os.path.join(folder_path, img)) for img in images]
    images[0].save(
        os.path.join(folder_path, f"{filename}.gif"),
        save_all=True,
...
def save_first_front(
    population: Population,
    fronts_indexes: Fronts,
    drone_size,
    first_label: str,
    second_label: str,
):
    ...
    for i, individual in enumerate(sorted_individuals):
        draw_experiment(
            ...
            [],
            save=True,
            subfolder="experiments/first_front",
            filename=f"individual_{i}",
